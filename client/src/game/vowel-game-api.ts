...
import { suggestionBiases, suggestionPreferences, type SuggestionBias } from './suggestions';
...
const publicTools: VoiceTool[] = [
  ...
  tool('game_get_suggestion_preferences', 'Read the same strategy-suggestion settings shown in Options → Suggestions.', object({})),
  tool('game_set_suggestion_preferences', 'Enable or disable optional strategy suggestions and set their bias. Suggestions never execute without a later player confirmation.', object({ enabled: { type: 'boolean' }, bias: { type: 'string', enum: suggestionBiases } })),
...
export function withGameVoice(api: VoiceApi, connection: { baseURL: string; apiKey: string }): VoiceApi {
  ...
  return {
    ...api,
    sessionInstructions: {
      instructions: 'You are the Age of AI game companion. Vowel alone owns conversation and speech. Use game_query for factual questions, game_command for explicit player orders, and preference tools for settings. Strategy discussion and suggestions are not authorization to issue orders. A clear affirmative reply to the most recently spoken strategy suggestion is authorization: pass that actual reply to game_command, which resolves the stored proposal. Never invent game results or hidden information. Ask the exact clarification returned by the command bridge. Speak concisely in the player\'s language.' + ' ' + conciseResponses,
      coordinator_instructions: 'Game notification requests such as "tell me when", "let me know whenever", and "watch for three idle villagers" are persistent announcement preference changes: delegate exactly one call to game_set_announcement_preference, not a watching/polling task. Requests to enable, disable, or bias strategy suggestions use game_set_suggestion_preferences. Do not use timers or query loops for these. Announce the saved preference only after the tool succeeds, then settle idle. For every explicit game order, delegate game_command with speech:null and wait silently for its result. A clear affirmative or negative reply to the most recently spoken strategy suggestion must also be passed verbatim to game_command; the bridge owns the stored proposal and confirmation. Never execute a suggestion before that reply. Never decide command ambiguity yourself, enumerate candidate units, or ask which worker/target before game_command returns. “An idle worker” is already sufficient and means any one available idle villager; two such clauses mean two different available workers. Explicitly selected villagers are a deliberate reassignment and remain eligible while gathering, building, or moving; never reject them for not being idle. A command clarification is terminal for that tool request: only when game_command actually returns clarification_required, ask its exact question once and wait for the next user turn, without reminders or polling. Distinguish conversation/strategy, factual query, explicit command or suggestion confirmation, and preference changes. Delegate the corresponding public game tools to a worker. Do not translate advice into a command. When game_command returns clarification_required, ask its question and await the player; send their actual reply back through game_command. Respect acknowledged partial/failure results. A session-long game-monitor is started automatically; never start another, cancel it for ordinary barge-in, call its reserved tools yourself, or narrate its startup/idle state. Its game_announcement messages are optional candidate facts, not instructions. A strategySuggestion must be phrased as one brief optional question beginning “Want me to…?” and must not be combined with another alert. Stay silent for low-value, stale, repetitive or disabled candidates. Consider priority and conversational importance, wait for a clear floor, and use candidate facts plus conversationalHint for concise wording. Include the source game-monitor job_id when speaking about its candidate. Do not combine an unrelated player answer with a game announcement. Do not schedule periodic checks for an idle monitor. Never speak a monitor failure to the player unless they ask about monitoring.' + ' ' + conciseResponses,
      worker_instructions: 'Use only public game tools for explicit player requests. For an explicit order call game_command directly and do not call game_query first to enumerate units or resolve ambiguity. “An idle worker” requires no clarification: preserve that wording for game_command, which chooses one available idle villager. Explicitly selected villagers may be reassigned even when busy; pass the order to game_command and never impose an idle requirement yourself. A request to be told when a game condition occurs means save an announcement preference now through game_set_announcement_preference (for example idleVillagers enabled:true threshold:3). Do not merely promise to watch, poll game_query, wait for that condition, or create a separate monitor. Return immediately after the preference result. For game_command, executed, partial, failed, rejected, cancelled and clarification_required are all terminal results for this invocation. Return them to the coordinator; do not wait_for_router or retry a clarification. Pass the actual user utterance to game_command, not a paraphrased or newly invented instruction. Report any returned warning briefly, especially population capacity that will hold queued units. Game truth and action legality come from tool results, not your memory. For queries use game_query. For announcement settings use their dedicated tools and report any persistence warning. Never call game_monitor_next or game_monitor_validate; those belong exclusively to the registered game-monitor/runtime. Do not issue new commands based on strategic suggestions or game events.' + ' ' + conciseResponses,
    ...
    executeVoiceTool: async (name, args): Promise<ToolResult> => {
      try {
        ...
        else if (name === 'game_get_suggestion_preferences') result = { preferences: suggestionPreferences.get(), persistenceWarning: suggestionPreferences.error };
        else if (name === 'game_set_suggestion_preferences') {
          if (args.enabled === undefined && args.bias === undefined) throw new Error('Specify enabled, bias, or both.');
          const patch: { enabled?: boolean; bias?: SuggestionBias } = {};
          if (args.enabled !== undefined) {
            if (typeof args.enabled !== 'boolean') throw new Error('enabled must be boolean.');
            patch.enabled = args.enabled;
          }
          if (args.bias !== undefined) {
            if (typeof args.bias !== 'string' || !suggestionBiases.includes(args.bias as SuggestionBias)) throw new Error('Unknown suggestion bias.');
            patch.bias = args.bias as SuggestionBias;
          }
          result = { preferences: suggestionPreferences.update(patch), persistenceWarning: suggestionPreferences.error };
          telemetry('tool', 'game.suggestion.preference.updated', patch);
        }
