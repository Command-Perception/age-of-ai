...
const publicTools: VoiceTool[] = [
  ...
  tool('game_command', 'Interpret the player\'s actual game order through Vowel decision inference, validate it, then issue it to the authoritative game server. Pass the actual utterance, not an invented order. This returns clarification_required for ambiguity, and executed/partial/failed outcomes. Never say success unless actual acknowledged results confirm it. Questions, hypotheticals and advice are NOT orders.', object({ utterance: string('The actual player game request or their reply to the pending clarification. Preserve their wording.') }, ['utterance'])),
